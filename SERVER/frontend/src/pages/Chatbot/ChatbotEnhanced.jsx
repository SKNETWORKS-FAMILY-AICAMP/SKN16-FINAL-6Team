 ...
 * 4. 통계 - 사용 패턴 분석
 *
 * @param {boolean} compact - 간소화 모드 (true: 메시지 + 입력만 표시, false: 전체 탭 표시)
 */
function ChatbotEnhanced({ compact = false }) {
  ...
  const [cachedRelatedQuestions, setCachedRelatedQuestions] = useState([])
  const [personalizationStatus, setPersonalizationStatus] = useState('unknown')
  ...
  const latestRelatedQuestions = useMemo(() => {
    const lastAssistant = [...messages].reverse().find(
      (msg) => msg.role === 'assistant' && msg.related_questions && msg.related_questions.length > 0
    )
    return lastAssistant ? lastAssistant.related_questions : []
  }, [messages])
  ...
  useEffect(() => {
    if (latestRelatedQuestions.length) {
      setCachedRelatedQuestions(latestRelatedQuestions)
      setPersonalizationStatus('ready')
    }
  }, [latestRelatedQuestions])
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const relatedQuestionsFromResponse =
          response?.data?.data?.related_questions ||
          response?.data?.related_questions ||
          response?.data?.metadata?.related_questions ||
          []

        if (relatedQuestionsFromResponse.length > 0) {
          setCachedRelatedQuestions(relatedQuestionsFromResponse)
          setPersonalizationStatus('ready')
        } else if (personalizationStatus === 'unknown') {
        ...
        const assistantMessage = {
          ...
          related_questions: relatedQuestionsFromResponse, // NEW!
  ...
  const handleToggleRelated = () => {
    if (!showRelated && latestRelatedQuestions.length) {
      setCachedRelatedQuestions(latestRelatedQuestions)
      setPersonalizationStatus('ready')
    }
  ...
  const handleCopy = async (content) => {
    try {
      // 방법 1: Modern Clipboard API (HTTPS 필요)
      if (navigator.clipboard && navigator.clipboard.writeText) {
        await navigator.clipboard.writeText(content)
        alert('클립보드에 복사되었습니다.')
      } else {
        // 방법 2: Fallback - 구형 브라우저 또는 HTTP 환경
  ...
  const dedupeSourcesByUrl = (sources = []) => {
    const seen = new Set()
  ...
  const extractTitleFromUrl = (url) => {
    try {
      ...
      const tail = parts.length ? decodeURIComponent(parts[parts.length - 1]) : parsed.hostname
      const formattedTail = tail.replace(/[-_]+/g, ' ').trim()
      return `${parsed.hostname}${formattedTail ? ` › ${formattedTail}` : ''}`
    } catch (e) {
      return url
    }
  }

  const getSourceTitle = (source, idx) => {
    const isPlaceholder = (val) => {
      if (!val) return true
      const normalized = String(val).trim().toLowerCase()
      return normalized === 'unknown document' || normalized === 'unknown' || normalized === 'source'
    }

    if (!source) return `출처 ${idx + 1}`
    if (typeof source === 'string') {
      if (isPlaceholder(source)) {
        return `출처 ${idx + 1}`
      }
      if (/^https?:\/\//i.test(source)) {
        return extractTitleFromUrl(source)
      }
      return source
    }

    let title =
      (!isPlaceholder(source.title) && source.title) ||
      (!isPlaceholder(source.document_title) && source.document_title) ||
      (!isPlaceholder(source.doc_title) && source.doc_title) ||
      (!isPlaceholder(source.name) && source.name) ||
      null

    if (title) return title
    if (source.url) return extractTitleFromUrl(source.url)
    if (source.chunk_id) return `문서 ${idx + 1}`
    return `출처 ${idx + 1}`
  }

  const getCategoryLabel = (category) => categoryLabels[category] || category || '일반 학습'
      ...
      <div className="chatbot-compact">
        <div className="compact-messages">
          {messages.map((message, index) => (
            <div key={index} className={`compact-message ${message.role}`}>
              <div className="compact-message-avatar">
                {message.role === 'user' ? '👤' : '🤖'}
              </div>
              <div className="compact-message-content">
                <div className="compact-message-text" style={{ whiteSpace: 'pre-wrap', lineHeight: '1.6' }}>
                ...
                {message.sources && message.sources.length > 0 && (
                  <div className="compact-sources">
                    <ul>
                      {dedupeSourcesByUrl(message.sources).slice(0, 3).map((source, idx) => (
                        <li key={idx}>
                          {typeof source === 'string' ? (
                            <span className="source-content">
                              <strong>{getSourceTitle(source, idx)}</strong>
                            </span>
                          ) : (
                            <>
                              <span className="source-content">
                ...
                {/* Compact Actions (Copy & Bookmark for assistant messages) */}
                {message.role === 'assistant' && (
                  <div className="compact-message-actions">
                    <button
          ...
          {loading && (
            <div className="compact-message assistant">
              <div className="compact-message-avatar">🤖</div>
              <div className="compact-message-content">
                ...
                <div className="loading-note">응답 생성 중...</div>
              </div>
            </div>
          )}
  ...
  const relatedList = cachedRelatedQuestions.length > 0 ? cachedRelatedQuestions : defaultExampleQuestions

  const renderChatTab = () => (
    <div className="chat-tab">
      <div className="chat-section">
        <div className="related-toggle-bar">
          <button className="related-toggle-btn" onClick={handleToggleRelated}>
            {showRelated ? '관련 질문 닫기' : '관련 질문 보기'}
          </button>
        </div>
        ...
        {showRelated && (
          <div className="example-questions">
            <div className="example-label">
              {cachedRelatedQuestions.length > 0 ? '💡 관련 질문' : '💡 예시 질문'}
            </div>
            {relatedList.map((question, index) => (
              <button
                key={index}
              ...
              <div className="message-content">
                ...
                {message.sources && message.sources.length > 0 && (
                  <div className="message-sources">
                    <ul>
                      {dedupeSourcesByUrl(message.sources).map((source, idx) => (
                        <li key={idx}>
                          {typeof source === 'string' ? (
                            <span className="source-content">
                              <strong>{getSourceTitle(source, idx)}</strong>
          ...
          {loading && (
            <div className="message assistant">
              ...
              <div className="message-content">
                ...
                <div className="loading-note">응답 생성 중... 잠시만 기다려주세요.</div>
              </div>
            </div>
          )}
          <div ref={messagesEndRef} />
          ...
          {sessions.map((session) => (
            <div key={session.id} className="session-item">
                <div className="session-info">
                  ...
                  {new Date(session.created_at).toLocaleDateString()} • {(session.message_count ?? session.messages?.length ?? 0)}개 메시지
                </div>
              </div>
      ...
      <div className="tab-header">
        ...
        <div className="history-filters">
          ...
          >
            ...
            <option value="unknown">일반 학습</option>
          </select>
        </div>
      </div>
            ...
            {history.map((item) => (
              <div key={item.id} className="history-item-full">
                ...
                <div className="history-answer">
                  ...
                  <span className="history-category">{getCategoryLabel(item.category)}</span>
          ...
          {analytics.favorite_category && (
            <div className="analytics-card">
              ...
              <div className="card-content">
                ...
                <div className="card-value">
                  {getCategoryLabel(
                    analytics.favorite_category.name ||
                    analytics.favorite_category.category ||
                    analytics.favorite_category.slug
                  )}
                </div>
              </div>
            </div>
