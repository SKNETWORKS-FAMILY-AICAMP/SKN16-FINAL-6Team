...
// Monaco Editor를 Lazy Loading으로 변경 (성능 최적화)
...
function CodingTest() {
  ...
  useEffect(() => {
    if (problemId) {
      // DB API를 통한 문제 로드 (최적화)
      api.get(`/coding-test/problems/${problemId}/`)
        .then(response => {
          if (response.data.success) {
            const foundProblem = response.data.data
      ...
      <div className="coding-test-content">
        ...
        <div className="code-panel" style={{ width: `${100 - leftWidth}%` }}>
          <div className="editor-section" style={{ height: `${editorHeight}%` }}>
            ...
            <Suspense fallback={<div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', color: '#fff' }}>에디터 로딩 중...</div>}>
              <Editor
