...
function Problems() {
  ...
  useEffect(() => {
    // DB API에서 문제 목록 로드 (최적화)
    api.get('/coding-test/problems/')
      .then(response => {
  ...
  const filteredProblems = problems.filter(problem => {
    // 상태 필터 (전체/푸는 중/0별/1별/2별/3별) - appliedFilters 사용
    const status = getProblemStatus(problem.problem_id)

    if (appliedFilters.statusFilter === 'in_progress' && status !== 'in_progress') return false
    if (appliedFilters.statusFilter === 'star_0' && status !== 'star_0') return false
    if (appliedFilters.statusFilter === 'star_1' && status !== 'star_1') return false
    if (appliedFilters.statusFilter === 'star_2' && status !== 'star_2') return false
    if (appliedFilters.statusFilter === 'star_3' && status !== 'star_3') return false

    const matchesCategory = appliedFilters.categories.length === 0 || appliedFilters.categories.includes(problem.category)
    const matchesLevel = appliedFilters.levels.length === 0 || appliedFilters.levels.includes(problem.level)
  ...
  const sortedProblems = [...filteredProblems].sort((a, b) => {
    if (!sortConfig.direction) return 0  // 정렬 없음

    let aValue = a[sortConfig.key]
    let bValue = b[sortConfig.key]

    // No. 컬럼은 숫자로 변환해서 정렬
    if (sortConfig.key === 'problem_id') {
      aValue = parseInt(aValue) || 0
      bValue = parseInt(bValue) || 0
    }

    // 별점 정렬을 위한 특수 처리
    if (sortConfig.key === 'star_count') {
      const aStatus = getProblemStatus(a.problem_id)
      const bStatus = getProblemStatus(b.problem_id)

      const getStarValue = (status) => {
        if (status === 'star_3') return 3
        if (status === 'star_2') return 2
        if (status === 'star_1') return 1
        if (status === 'in_progress') return -1
        return 0
      }

      aValue = getStarValue(aStatus)
      bValue = getStarValue(bStatus)
    }
        ...
        <table className="problems-table">
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort('problem_id')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                No.{getSortIcon('problem_id')}
              </th>
              <th onClick={() => handleSort('title')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                문제명{getSortIcon('title')}
              </th>
              <th onClick={() => handleSort('level')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                단계{getSortIcon('level')}
              </th>
              <th onClick={() => handleSort('category')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                분류{getSortIcon('category')}
              </th>
              <th onClick={() => handleSort('star_count')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                별점{getSortIcon('star_count')}
              </th>
              <th></th>
            </tr>
          </thead>
