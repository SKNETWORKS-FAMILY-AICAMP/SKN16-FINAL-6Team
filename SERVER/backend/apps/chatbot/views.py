...
def _resolve_user_session(session_identifier, user):
    """
    session_identifier가 UUID 문자열이든 숫자 ID든 사용자 소유 세션을 찾아 반환한다.
    """
    ...
    try:
        ...
        return ChatSession.objects.get(session_uuid=uuid_obj, user=user)
    except (ValueError, ChatSession.DoesNotExist):
        pass

    # 2) 숫자 ID 매칭 (기존 호환용)
    try:
        return ChatSession.objects.get(id=int(identifier), user=user)
    except (ValueError, TypeError, ChatSession.DoesNotExist):
        return None


def _to_comma_separated(value) -> str:
    """
    리스트는 콤마로 이어붙이고, 기타 타입은 문자열로 강제 변환한다.
    빈 값은 빈 문자열을 반환한다.
    """
    if not value:
        return ""
...
def chat(request):
    ...
    if session_identifier:
        session = _resolve_user_session(session_identifier, request.user)
        if not session:
            return Response({
                'success': False,
                'error': '세션을 찾을 수 없습니다.'
            }, status=status.HTTP_404_NOT_FOUND)
        logger.info(f"[Chat] Using existing session: {session.id} ({session.session_uuid})")
    else:
        # 새 세션 생성
        ...
        logger.info(f"[Chat] New session created: {session.id} ({session.session_uuid})")

    session_external_id = str(session.session_uuid)
    ...
    try:
        ...
        if survey:
            learning_goals_str = _to_comma_separated(survey.learning_goals)
            interested_topics_str = _to_comma_separated(survey.interested_topics)
...
def get_session_history(request, session_id):
    """특정 세션의 채팅 내역 조회"""
    session = _resolve_user_session(session_id, request.user)
    if not session:
        return Response({
            'success': False,
            'error': '세션을 찾을 수 없습니다.'
