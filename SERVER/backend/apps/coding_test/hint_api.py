...
"""
    ...
    elif ai_config.mode == 'openai':
        # OpenAI API 모드: OpenAI SDK를 사용하여 요청
        try:
            from openai import OpenAI

            if not ai_config.openai_api_key:
                hint_response = "OpenAI API 키가 설정되지 않았습니다. 관리자에게 문의하세요."
            else:
                # OpenAI 클라이언트 생성
                # runpod_endpoint가 있으면 런팟 사용, 없으면 OpenAI 공식 API 사용
                if ai_config.runpod_endpoint:
                    # 런팟 엔드포인트 사용 (OpenAI 호환 API)
                    client = OpenAI(
                ...
                # 시스템 프롬프트
                system_prompt = """당신은 코딩 교육 전문가입니다. 12개 지표를 모두 반영하여 JSON 형식으로 힌트를 반환해야 합니다."""
            ...
            hint_response = f"OpenAI API 오류: {str(e)}. API 키와 네트워크 연결을 확인하세요."
