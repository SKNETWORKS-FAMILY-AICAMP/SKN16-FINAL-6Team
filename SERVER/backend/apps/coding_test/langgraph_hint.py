...
class HintState(TypedDict):
    ...
    code: str  # COH 코드 해시 계산용 (user_code와 동일)
    code_hash: str  # 사용자 코드의 해시값 (COH 추적용)
    coh_decision: str  # COH 결정 이유
...
def llm_eval_node(state: HintState) -> HintState:
    ...
    try:
        ...
        # Runpod 엔드포인트가 있으면 런팟 사용, 없으면 OpenAI 공식 API 사용
        if ai_config.runpod_endpoint:
            client = OpenAI(
                base_url=f"{ai_config.runpod_endpoint}/v1",
                api_key=ai_config.openai_api_key or "EMPTY"
            )
        else:
...
def coh_check_node(state: HintState) -> HintState:
    ...
    try:
        ...
        if prev_code_hash and prev_code_hash != current_code_hash:
            # 코드 변경됨 → COH 리셋 (새로운 시도)
            state['coh_depth'] = 0
            state['coh_decision'] = 'reset_code_changed'
            return state
...
def run_langgraph_hint(user, problem_id, user_code, preset='중급', custom_components=None, previous_hints=None):
    ...
    # 이제 HINT_EXECUTION_MODE 환경변수를 사용하지 않음
    # 항상 Django 로컬에서 LangGraph를 실행
    # LangGraph 내부 노드들은 AIModelConfig의 mode='openai' + runpod_endpoint 설정을 따름
    if not LANGGRAPH_AVAILABLE:
        return (
            False,
            None,
